...
const InputForm: React.FC<InputFormProps> = ({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -20 }}
      className="relative overflow-visible glass rounded-2xl sm:rounded-3xl lg:rounded-[32px] p-4 sm:p-6 lg:p-8 xl:p-10 border border-white/10 shadow-xl"
    >
      ...
      >
          ...
          <div className="flex bg-surface-dark border border-white/5 rounded-xl sm:rounded-2xl p-1 sm:p-1.5 gap-1 sm:gap-1.5 h-[52px] sm:h-[60px] lg:h-[68px]">
            {["Nam", "Nữ", "Khác"].map((g) => (
              <motion.button
                ...
                className={`flex-1 h-full rounded-lg sm:rounded-xl text-xs sm:text-sm font-black uppercase tracking-widest transition-all flex items-center justify-center ${
                  formData.gender === g
        ...
        {/* Date of Birth */}
        <motion.div
          className="md:col-span-2 space-y-2 sm:space-y-2.5"
          variants={fadeInUp}
        >
          <label className="text-[10px] font-black uppercase tracking-[0.2em] text-primary ml-1 block text-center md:text-left">
            NGÀY THÁNG NĂM SINH
          </label>
          <BirthDatePicker
            value={formData.dob}
        ...
        {/* Time of Birth */}
        <motion.div
          className="md:col-span-2 space-y-2 sm:space-y-2.5 overflow-visible"
          variants={fadeInUp}
        >
          <label className="text-[10px] font-black uppercase tracking-[0.2em] text-primary ml-1">
            Giờ sinh chi tiết
          </label>
          <div className="flex items-center bg-background-dark/60 border border-white/5 rounded-xl sm:rounded-2xl p-1 gap-3 sm:gap-4 focus-within:border-primary/50 transition-all overflow-visible">
            <div className="w-full overflow-visible">
              <BirthTimePicker
                value={formData.tob}
                onChange={(time) => onChange({ tob: time })}
              />
            </div>
          </div>
        </motion.div>
